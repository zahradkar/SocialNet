...
public class OpenGraph {
	...
	public final static Hashtable<String, String[]> BASE_TYPES = new Hashtable<>();
	...
	private final ArrayList<OpenGraphNamespace> pageNamespaces;
	private final Hashtable<String, ArrayList<MetaElement>> metaAttributes;
	private final boolean hasChanged; // track if object has been changed
	...
	public OpenGraph(String url, boolean ignoreSpecErrors) throws java.io.IOException, Exception {
		...
		if (!hasOGspec)
			pageNamespaces.add(new OpenGraphNamespace("og", "https://ogp.me/ns#"));
		...
		if (currentType != null) {
			for (OpenGraphNamespace ns : pageNamespaces) {
				if (currentType.startsWith(ns.prefix() + ":")) {
					currentType = currentType.replaceFirst(ns.prefix() + ":", "");
					break; // done here
				}
			}
		}
	...
	public String getContent(String property) {
		if (metaAttributes.containsKey(property) && !metaAttributes.get(property).isEmpty())
			return metaAttributes.get(property).get(0).content();
		else
			return null;
	}
	...
	public String[] toHTML() {
		...
		// keep track of the index to insert into
		for (ArrayList<MetaElement> elements : metaAttributes.values()) {
			for (MetaElement element : elements)
				returnHTML.add("<meta property=\"" + element.namespace() + ":" +
						element.property() + "\" content=\"" + element.content() + "\" />");
		}
	...
	public String[] toXHTML() {
		...
		for (ArrayList<MetaElement> elements : metaAttributes.values()) {
			for (MetaElement element : elements)
				returnHTML.add("<meta name=\"" + element.namespace().prefix() + ":" +
						element.property() + "\" content=\"" + element.content() + "\" />");
		}
	...
	public void setProperty(OpenGraphNamespace namespace, String property, String content) {
		...
		property = property.replaceAll(namespace.prefix() + ":", "");
